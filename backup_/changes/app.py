...
# import pymysql
from flask import Blueprint, Flask, jsonify,render_template, request,flash, send_file
...
def admin_render():
        ...
        cursor.execute("select name,address, contact_no, language, CONCAT_WS( ', ', ty_gn,ty_dr,ty_fl,ty_wal,ty_wel,ty_cnc,ty_wh,ty_of ) from users") 
...
def upload_files():
    ...
    try:    
        ...
        if uploaded_file:
            ...
            if upload_id:
                cursor = mysql.connection.cursor()
                cursor.execute(''' INSERT INTO users(phn, name, address, dob, email, gender, language, prv_em, year_of_job, job_add, jobnature, ty_gn, ty_gn_hy1, ty_dr, ty_gn_hy2, ty_fl, ty_gn_hy3, ty_wal, ty_gn_hywal, ty_wel, ty_gn_hy4, ty_cnc, ty_gn_hy5, ty_wh, ty_gn_hy6, ty_of, ty_of_hy7, contact_no) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)''',(ph_no_key,full_name,address,dob,email,gender,lang,prev_em,year_of_job,job_add,jobNature,typeGeneral,typeGeneralhistory,typeDriving,typeGeneralhistory1,typeForklift,typeGeneralhistory2,typeWalkie,typeGeneralhistory3,typeWelder,typeGeneralhistory4,typeCNC,typeGeneralhistory5,typeWarehouse,typeGeneralhistory6,typeOffice,typeOfficehistory7,ph_no,))
...
def getcusdata():
    try:
        if request.method == "POST":
            ...
            if qtc_data['job'] == "All":
                if lang == "":
                    cmd = "select name,address, contact_no, language, CONCAT_WS( ', ', ty_gn,ty_dr,ty_fl,ty_wal,ty_wel,ty_cnc,ty_wh,ty_of ) from users"
                else:
                    cmd = "select name,address, contact_no, language, CONCAT_WS( ', ', ty_gn,ty_dr,ty_fl,ty_wal,ty_wel,ty_cnc,ty_wh,ty_of ) from users where"+ lang
            else:                
                ...
                cmd = "select name, address, contact_no, language"+jobc+" from users "+sjobc+lang
            print(cmd)
            cursor = mysql.connection.cursor()
            cursor.execute(cmd)
            data = cursor.fetchall()   
